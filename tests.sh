...
for FOLDER in */; do
    ...
    for i in $(seq $count); do
        ...
        if [ -x "$(command -v elixirc)" ]; then
            if test -f "$FOLDER/elixir.exs"; then
                num=$(elixirc "$FOLDER/elixir.exs" --ignore-module-conflict)
                num=$(trim_ms $num)
                
                if [[ -v tests[elixir] ]]; then
                    prev="${tests[elixir]}"
                    tests[elixir]=`echo "$prev + $num" | bc`
                else
                    tests[elixir]=$num
                fi
            fi
